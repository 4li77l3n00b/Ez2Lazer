    ...
    public static class KrrLNConverter
    {
        ...
        public static void Transform(ManiaBeatmap beatmap, KrrLNOptions options)
        {
            ...
            if (options.Alignment.HasValue)
                KrrConversionHelper.ApplyAlignmentToCandidates(merged, beatmap, options.Alignment.Value);
            ...
            // OD 覆盖
            if (options.ODValue.HasValue)
                beatmap.Difficulty.OverallDifficulty = options.ODValue.Value;
        }

        private static List<(int index, double start, int length)> enforceLimit(
            List<(int index, double start, int length)> list, int limit, Random rg)
        {
